/*
  ...
  1. New Types
    - `quote_status` enum for tracking quote states
    - `quote_template_type` enum for template categories
    - `quote_section_type` enum for template section types

  2. New Tables
    - `quote_templates`: Store reusable quote templates
    - `quote_sections`: Template building blocks
    - `quotes`: Main quote records
    - `quote_items`: Line items in quotes
    - `quote_variables`: Dynamic content variables
    - `quote_history`: Status change tracking
...
-- Create quote_variables table
CREATE TABLE quote_variables (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  quote_id uuid REFERENCES quotes(id) ON DELETE CASCADE,
  name text NOT NULL,
  value text,
...
-- Create function to create quote from template
...
-- Create function to update quote status
