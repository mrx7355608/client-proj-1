...
import { LayoutDashboard, Users, Quote, Settings as SettingsIcon, LogOut, DollarSign, Handshake, UserCircle, UserCog, Package, FolderOpen } from 'lucide-react';
    ...
    <Router>
      <div className="min-h-screen bg-gray-50 flex">
        ...
        <nav className="w-48 bg-white shadow-lg flex flex-col h-screen fixed z-30">
          <div className="p-4">
            <h1 className="text-xl font-bold text-gray-800">Client Manager</h1>
          </div>
          <div className="flex-1 overflow-y-auto">
            <NavLink to="/" icon={<LayoutDashboard size={18} />} text="Dashboard" />
            <NavLink to="/clients" icon={<Users size={18} />} text="Clients" />
            <NavLink to="/partners" icon={<Handshake size={18} />} text="Partners" />
            <NavLink to="/proposals" icon={<Quote size={18} />} text="Proposals" />
            <NavLink to="/inventory" icon={<Package size={18} />} text="Inventory" />
            <NavLink to="/documents" icon={<FolderOpen size={18} />} text="Documents" />
            <NavLink to="/expenses" icon={<DollarSign size={18} />} text="Global Expenses" />
            {userProfile?.is_admin && (
              <NavLink to="/users" icon={<UserCog size={18} />} text="Users" />
            )}
            <NavLink to="/settings" icon={<SettingsIcon size={18} />} text="Settings" />
          </div>
          ...
          <Routes>
            ...
            {userProfile?.is_admin && (
              <>
                ...
                <Route path="/users/:id/settings" element={<UserSettingsManagement />} />
              </>
            )}
          </Routes>
        </main>
